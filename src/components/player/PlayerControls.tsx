  ...
  return (
    <div className="flex items-center justify-center gap-6">
      ...
      <button onClick={onPlayPause} className="p-4 bg-white rounded-full hover:scale-110 transition-transform shadow-lg">
        {isPlaying ? <img src={Pause} alt={getAltFromPath(Pause)} width={28} className="text-black" /> : <img src={Play} alt={getAltFromPath(Play)} width={28} className="text-black fill-black" />}
      </button>
      <button onClick={onNext} className="p-3 hover:bg-white/10 rounded-full transition-colors">
        <img src={SkipForward} alt={getAltFromPath(SkipForward)} width={24} className="text-white" />
      </button>
      {onRepeat && (

...
const SearchPage: FC = () => {
  ...
  const handleSearch = async (query: string = debouncedQuery, customFilter = filter) => {
    setLoading(true);
    try {
      let promises: Promise<UserProfile[] | SearchResult>[] = [
        profileApi.searchStalkers(query.replaceAll(' ', '%20')),
      ];
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 row-reverse">
          {filters.map((f) => (
            <button
              ...
              className={`flex min-w-fit items-center gap-2 px-2 py-2 rounded-full whitespace-nowrap transition-all ${
                filter === f.value
            ...
            >
              {f.icon}
              <span className="flex-1 flex">{f.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400">
            {recentSearches.length ? (
              <div className="flex flex-col gap-4 ">
                {recentSearches
                  .filter((e) => (filter != SearchFilters.all ? e.filter == filter : true))
                  .map((e) => (
                    <div className="flex justify-between flex-1" key={e.id}>
                      <div
                      ...
                      <img
                        ...
                        width={18}
                        className=" mb-4 mx-5 opacity-50"
                      />
                    </div>
                  ))}
              </div>
          ...
          <div className="space-y-8 pb-6">
            {[SearchFilters.all, SearchFilters.artist].includes(filter) &&
                    ...
                    {results.artist.map((artist) => (
                      <div className="min-w-36 max-w-36">
                        <ArtistCard
                          key={artist.id}
                          artist={artist}
                          onClick={() => navigate(buildPath(routes.artist, { id: artist.id }))}
                        />
                      </div>
                    ))}
                  </div>
            ...
            {[SearchFilters.all, SearchFilters.stalker].includes(filter) &&
              results?.stalker?.length > 0 && (
                <div>
                  <h2 className="text-xl font-bold text-white mb-4">Users</h2>
                  <div className="space-y-2">
                    {results.stalker.map((user) => (
                      <UserCard key={user.id} user={user} onStalk={() => {}} onView={() => {}} />
                    ))}
                  </div>
                </div>
              )}

            {[SearchFilters.all, SearchFilters.album].includes(filter) &&
            ...
            {[SearchFilters.all, SearchFilters.track].includes(filter) &&

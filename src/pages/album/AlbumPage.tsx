...
import { Play, Shuffle, X } from '@/assets/svg';
...
const AlbumPage: FC = () => {
  ...
  const handleShuffle = () => {
    if (album?.songs?.length) {
      const firstSong = album?.songs[Math.floor(Math.random() * album?.songs.length)];
      setShuffle(
        true,
        album?.songs.filter((e) => e.id != firstSong.id)
      );
      play(firstSong);
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-gradient-to-b from-gray-900 via-purple-900/20 to-gray-900 overflow-auto">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col items-start gap-6 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-400">
              {album.releaseDate} / {album.songs?.length} tracks
            </p>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <SongList
            songs={album?.songs?.sort((a, b) => b.externalListens - a.externalListens) || []}
            onPlay={onPlay}
          />
        </div>
      </div>
